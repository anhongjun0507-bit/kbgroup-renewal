...
**프로덕션 배포 완료 · 최종 배포 `dpl_7tLisAFT1m6rt2AghkMTBov8CbCw` (2026-08-26).**
PLAN B 전 작업(ITEM 01~04 + 복구 UI)이 라이브에 올라갔다 — 직전 프로덕션 배포가 2026-07-29 였으므로
이번이 **PLAN B 최초 배포**이고, sitemap 조치 시도로 한 번 더 배포해 총 2회다.
`kbgroup.kr` · `www.kbgroup.kr` · `kbgroup-renewal.vercel.app` 전부 최종 배포를 가리킨다.
라이브 검증: **SSR 가시 텍스트 20경로 diff 0줄** · 관리자 탭 11개 · sitemap 191 · 단지 153/19 ·
채용 3건 · 히어로 8슬라이드 · 편집→반영 왕복 · 페이지 비공개 시 **404 즉시(1초)**.

**알려진 동작 1건** — 페이지 비공개 시 `/sitemap.xml` 반영만 최대 1시간 지연된다(§22).
`revalidatePath("/sitemap.xml")` 조치를 시도했으나 프로덕션에서 효과가 없었고, 광역 무효화는
쓰지 않기로 했다(E-12). 404·메뉴 제외는 즉시 걸리므로 실피해는 낮다. **문서화하고 인수한다.**
§11-5 잔여 항목은 이로써 **종결**.
...
- [x] DAY 9 — 채용 공고 마감(ITEM 04) + 게시판 카테고리 + 공지 상단 고정 + `content_revisions` 복구 UI (§19)
- [x] DAY 10 — 전체 회귀 전수 검증 + 인수 문서 3종 (§20). **프로덕션 배포는 사용자 몫 — 미실행**
...
### 11-5. 프로덕션 제약은 로컬에서 드러나지 않는다 — **종결 (2026-08-26)**

> **잔여 항목이던 「`updateTag` 의 sitemap 전파」는 §22 에서 판정이 끝났다. 결론: 전파되지 않는다.**
> `revalidatePath("/sitemap.xml")` 를 더해 재배포해도 마찬가지였다. 이 절의 경고가 실례로 확인된 셈이라
> 「미확인 잔여 항목」 상태를 닫는다. 남은 것은 결함이 아니라 **문서화된 알려진 동작**이다.
...
## 19. DAY 9 실행 결과 (2026-08-26)

> ITEM 04 「게시판 · 채용 관리」 + 복구 UI. 지시서 9-1 ~ 9-4.
> 전 수치는 **로컬 프로덕션 빌드**(`localhost:3210`) 실측이다.

### 19-0. 착수 시점 사실 확인 — 채용·공지 고정은 이미 있었다

지시 9-1·9-2 를 들어가기 전에 코드를 먼저 읽었다. **상당 부분이 2026-06 작업으로 이미 구현돼 있었다.**

| 지시 항목 | 착수 시점 상태 |
|---|---|
| 채용 공고를 DB 로 이전 | **이미 완료** — `job_openings` 테이블(마이그레이션 `20260608000003`) · `lib/job-openings.ts` |
| 등록 / 수정 편집 UI | **이미 완료** — `app/admin/openings/*` (목록·신규·수정·삭제·노출 토글) |
| 공지 상단 고정 | **이미 완료** — `posts.is_pinned` + 관리자 토글 + 목록 정렬(`lib/posts.ts:124`) |
| 마감된 공고 자동 제외 | **없었다** — `getPublishedOpenings()` 가 `is_published` 만 봤다 |
| 게시판 카테고리 관리 | **없었다** |
| `content_revisions` 복구 UI | **없었다** (적재는 DAY 3·4 부터 동작 중) |

**정정 1 — 지시서의 「관리소장 수시채용 1건」은 현재 사실과 다르다.**
DB·라이브 모두 **3건**이다: 관리소장(2026-06-08 등록) · 경비원 · 미화원(둘 다 06-18).
셋 다 `deadline = null`(상시채용)이라 이번 마감 필터에 걸리지 않는다.
「기존 렌더가 그대로 나와야 한다」는 요구는 **3건 그대로 유지**로 읽고 검증했다(§19-4).

### 19-1. 설계 결정

**결정 A — 마감은 두 갈래로 두되 판정 기준은 한 곳에서만 만든다.**
`is_published`(관리자가 즉시 내림)와 `deadline`(날짜가 지나 자동으로 빠짐) 두 경로가 있다.
공개 목록에서 빠지는 조건은 `lib/jobs.ts` 의 `deadlineBadge(...).tone === "closed"` 하나로 통일했다
(`isDeadlinePassed`). 뱃지가 「마감」이라고 표시하면서 목록에는 남아 있는 모순이 생기지 않는다.
관리자 화면 라벨도 「비공개로 → 마감 처리」, 「공개/비공개 → 게시중/마감」으로 계약 문구에 맞췄다.

**결정 B — 게시판 카테고리는 「이름 · 설명」까지다. 순서·표시숨김은 넣지 않는다.**
지시 9-2 는 「현재 스키마 확인 후 최소 구현, 기존 글이 깨지는 스키마 변경 금지」였다.

- **추가·삭제 불가** — `posts.board_type` 이 CHECK 제약으로 4종에 묶여 있고, 종류를 늘리면
  기존 글의 라우트·첨부 정책(버킷·MIME·개수)까지 함께 건드려야 한다. 범위 밖이다.
- **순서·표시숨김 제외** — 방문자가 보는 게시판 목록은 헤더 「소식」 드롭다운뿐이고,
  그건 이미 DAY 8 의 「메뉴 구성」(`nav_items`)이 관리한다. 여기에 또 만들면
  **DAY 8 결정 2 에서 피했던 두 출처 문제**가 그대로 재현된다. 관리 화면에 안내 링크만 뒀다.
- 남은 것이 이름·설명이고, 그것을 `site_settings.boardCategories` 한 행에 오버레이로 담았다.

**결정 C — 게시판 오버레이도 「코드가 원본, DB 는 그 위」 원칙 그대로.**
원본은 `lib/boards.ts` 의 `BOARD_CONFIGS`. 값이 없으면 전환 전과 한 글자도 다르지 않다.
입력값이 코드 기본값과 같으면 **저장 시 키 자체를 넣지 않는다** — 나중에 코드 문구가 바뀌어도
옛 값이 유령처럼 남지 않는다. 관리 화면에서 칸을 비우는 것이 곧 초기화다.

**결정 D — 목록 히어로 문구는 페이지가 기본값을 갖는다.**
게시판 4종 중 **공지사항만** 목록 히어로 문구(`(주)케이비개발의 공지·소식을 확인하세요.`)가
`BOARD_CONFIGS.subtitle`(`…공지·안내를 전달드립니다.`)과 다르다. 통일하면 SSR diff 가 난다.
그래서 목록 페이지가 `subtitleOverride ?? "<자기 문구>"` 로 받는다 — 관리자가 설정하면 목록·상세
양쪽에 적용되고, 설정하지 않으면 각 위치의 기존 문구가 그대로 유지된다.

**결정 E — 복원도 새 리비전을 남긴다(되돌리기의 되돌리기).**
`restoreRevision()` 은 ① 지금 값을 스냅샷으로 적재 → ② 스냅샷 값으로 덮어쓰기 순서로만 실행한다.
대상이 이미 삭제됐으면 **원래 `id`·`slug` 그대로 다시 넣는다** — 단지를 잘못 지웠을 때
기존 URL(`/cases/[slug]`)까지 살아난다(E-1). `slug` 는 UPDATE 트리거가 막으므로 갱신 payload 에서 뺐다.

**결정 F — 복원 대상은 `complexes` · `site_settings` 두 테이블뿐.**
`content_revisions.table_name` 은 text 라 무엇이든 담길 수 있다. 화이트리스트 밖 값은 거부한다.

### 19-2. 신규·변경 파일

| 구분 | 파일 | 내용 |
|------|------|------|
| 신규 | `lib/board-categories.ts` | 게시판 오버레이 리더. 태그 `content:boards`. JSONB 드리프트 정규화(E-9) |
| 신규 | `lib/content/labels.ts` | 설정 키 → 한글 라벨. 설정 저장 메시지와 복구 화면이 공용 |
| 신규 | `lib/content/revisions.ts` | 이력 조회 · 편집자 매핑 · 복원(설정/단지, 삭제분 되살리기 포함) |
| 신규 | `app/admin/content/boards/{page.tsx,actions.ts}` | 게시판 카테고리 편집 UI |
| 신규 | `app/admin/content/revisions/{page.tsx,actions.ts}` | 복구 이력 목록 · 스냅샷 목록 · 복원 |
| 신규 | `components/admin/BoardCategoryForm.tsx` | 4개 게시판 한 폼 저장 |
| 신규 | `scripts/verify-day9.mjs` | 실동작 검증 37항목 |
| 변경 | `lib/jobs.ts` | `isDeadlinePassed` · `todayKst` 추가 |
| 변경 | `lib/job-openings.ts` | `getPublishedOpenings` 에 `deadline.is.null,deadline.gte.오늘(KST)` 필터 |
| 변경 | `app/admin/openings/page.tsx` | 게시중/마감 배지 · 「기한 경과」 안내 · 버튼 라벨 |
| 변경 | `app/notices/**` 8개 | 목록 4 + 상세 4 가 오버레이를 얹어 읽음 |
| 변경 | `app/admin/posts/**` 4개 | 관리자 화면에도 바뀐 게시판 이름 반영 |
| 변경 | `components/admin/AdminTabs.tsx` | 탭 9 → **11** (게시판 카테고리 · 복구 이력). 11개라 `flex-wrap` 적용 |
| 변경 | `lib/content/tags.ts` | `boards: "content:boards"` 추가 |
| 변경 | `app/admin/content/settings/actions.ts` | 내부 `LABELS` 상수 → `lib/content/labels.ts` 로 추출 |
| 변경 | `scripts/snapshot-ssr-text.mjs` | 대조 경로 16 → **20** (게시판 3 + `/careers/openings`) |
| 변경 | `scripts/seed-content.ts` | 딥 비교에서 오버레이 전용 키(`boardCategories`) 제외 |

`posts` · `job_openings` 테이블 스키마는 **한 줄도 바꾸지 않았다.** 마이그레이션 추가 없음.

### 19-3. 검증 ① — SSR 가시 텍스트 전문 diff (20경로)

같은 로컬 서버에서 「DAY 9 착수 전 코드(day9-before)」 ↔ 「DAY 9 코드(day9-after)」.

| 경로 | 줄 수 | diff | | 경로 | 줄 수 | diff |
|------|--:|--:|---|------|--:|--:|
| `/` | 164 | **0** | | `/cases` | 223 | **0** |
| `/about` | 194 | **0** | | `/licenses` | 191 | **0** |
| `/about/ceo` | 71 | **0** | | `/careers` | 120 | **0** |
| `/about/history` | 79 | **0** | | `/careers/openings` | 65 | **0** |
| `/about/location` | 74 | **0** | | `/contact` | 62 | **0** |
| `/business` | 87 | **0** | | `/notices` | 53 | **0** |
| `/business/facility` | 109 | **0** | | `/notices/gallery` | 54 | **0** |
| `/business/sanitation` | 112 | **0** | | `/notices/news` | 48 | **0** |
| `/business/security` | 110 | **0** | | `/notices/resources` | 55 | **0** |
| `/business/construction` | 110 | **0** | | `/business/others` | 111 | **0** |

**20경로 전부 diff 0줄.**

> **함정 기록** — 1회차 대조는 **무효였다.** `next start` 가 이미 점유된 3210 포트에서
> `EADDRINUSE` 로 조용히 죽어(백그라운드 실행이라 눈에 안 띈다) 옛 빌드 서버를 그대로 읽었다.
> 즉 before ↔ before 를 비교하고 「diff 0」을 받은 것이다. 서버 시작 시각과 `.next/BUILD_ID`
> mtime 이 어긋나 발견했다. **이후 대조는 서버 로그의 `✓ Ready` 확인 + 신규 라우트 응답 확인 후에 돌렸다.**

### 19-4. 검증 ② — 실동작 37항목 (`scripts/verify-day9.mjs`)

실제 브라우저로 관리자 버튼을 **클릭**하고 공개 페이지를 HTTP 로 다시 읽는다. 전부 원복까지 돈다.

**채용 공고 — 등록 → 마감 → 삭제 왕복**

| 항목 | 결과 |
|------|------|
| 공고 등록 → DB 적재 · 관리자 목록 +1 (3 → 4) | PASS |
| 공고 등록 → `/careers/openings` 노출 | PASS |
| 「마감 처리」 → 관리자 배지 `data-status=closed` | PASS |
| 마감 처리 → **관리자 화면에 잔존** (1행) | PASS |
| 마감 처리 → `/careers/openings` 에서 제외 | PASS |
| 마감 처리 → `/careers` 섹션에서도 제외 | PASS |
| 삭제 → 원복 | PASS |

**채용 공고 — 마감일 경과 자동 제외 (지시 9-1 핵심)**

| 항목 | 결과 |
|------|------|
| `deadline` 어제 · `is_published=true` → **공개 목록에서 자동 제외** | PASS |
| 같은 공고가 관리자 화면에는 잔존 + 「기한 경과」 안내 | PASS |
| `deadline` 을 +30일로 바꾸면 공개 목록 복귀 | PASS |
| 왕복 후 `/careers` · `/careers/openings` diff **0줄** | PASS |

**공지 상단 고정**

| 항목 | 결과 |
|------|------|
| 「상단 고정」 → `is_pinned=true` | PASS |
| 목록 최상단 이동 (고정글 index 29 · 나머지 최소 index 30) | PASS |
| 「고정 해제」 → 원복 | PASS |
| 왕복 후 `/notices` diff **0줄** | PASS |
...
|------|------|
| 관리 화면 4행 렌더 | PASS |
| 단지소식 이름 「단지 이야기」 → 목록 페이지 반영 | PASS |
| 설명 문구 변경 → 목록 페이지 반영 | PASS |
| 이름 변경 시 **줄 수 유지** (48줄) — 구조 변화 없음 | PASS |
| 다른 게시판(`/notices/gallery`) 무영향 diff **0줄** | PASS |
| 칸을 비우면 기본값 복귀 → diff **0줄** | PASS |
...
|------|------|
| 단지 1건 수정 반영 | PASS |
| 복구 이력 상세 200 · 스냅샷 목록 · **편집자 표시** | PASS |
| 「이 시점으로 복원」 → **원값 일치** (`region=전남광주특별시 동구`) | PASS |
| 복원 직전 값도 적재 → **되돌리기의 되돌리기 가능** (리비전 5건) | PASS |
| 복원 후 `/cases` diff **0줄** | PASS |

**잔존물** — 검증용 공고 0건 · 게시판 오버레이 0건 · 상단 고정 0건. **전부 원복 확인.**

> **함정 기록** — 1회차에서 6건이 FAIL 났다. 원인 둘 다 검증 스크립트 쪽이었다.
> ① `page.click('button[type="submit"]')` 가 폼이 아니라 **헤더 모바일 메뉴의 LOGOUT** 을 눌렀다
> (그 버튼이 DOM 상 먼저다). 이후 라벨(`공고 등록`·`변경 저장`)로 집도록 고쳤다.
> ② Server Action 은 응답을 돌려준 뒤에도 캐시 무효화가 이어져서, `networkidle` 직후에 읽으면
> 이전 값이 잡힌다. 조건이 참이 될 때까지 다시 읽는 `until()` 로 감쌌다. **제품 결함이 아니다** —
> 같은 판정 근거로 「목록 최상단 이동」은 1회차에서도 PASS 였다.

### 19-5. 검증 ③ — 스크린샷 28장 재캡처

DAY 8 `day8-after` ↔ DAY 9 `day9-after`. **28장 전부 크기(높이) 동일 — 레이아웃 변화 0.**

**픽셀 동일 14장** — `03_about-ceo`(2) · `04_about-history`(2) · `05_about-location_desktop` ·
`08_licenses_mobile` · `09_careers_desktop` · `10_contact`(2) · `11_notices`(2) ·
`12_cases-detail-lh`(2) · `13_cases-detail-private_desktop` · `14_cases-detail-nophoto`(2).

나머지 14장은 **지연 로딩 캡처 아티팩트**다. 가장 큰 `07_cases_desktop`(36.21%)을 수치로 확인했다:

- 차이 구간이 **카드 격자 주기와 일치** — 밴드 간격 361px · 485px 의 반복 블록 12개.
- **어느 쪽이 비어 있는지가 한 장 안에서도 갈린다** — 회귀라면 한 방향으로만 나와야 한다.

| 구간 | DAY 8 | DAY 9 |
|------|-------|-------|
| y1433–1778 | mean 162.8 / std 77.5 (사진 있음) | **mean 255.0 / std 0.0 (순백)** |
| y1794–2139 | mean 148.4 / std 80.5 | **순백** |
| y2913–3366 | **mean 248.3 / std 1.2 (거의 순백)** | mean 173.5 / std 69.9 (사진 있음) |
| y3398–3851 | **거의 순백** | 사진 있음 |

같은 이미지 쌍 안에서 빈 쪽이 뒤집힌다 = 캡처 시점의 이미지 로드 진행도 차이다.
**같은 경로의 SSR 가시 텍스트 diff 가 0줄**이라는 사실이 콘텐츠 손실이 아님을 한 번 더 뒷받침한다.

> `05_about-location` 2장은 1회차 캡처가 `networkidle` 대기 중 타임아웃으로 빠졌다.
> `ONLY=05` 로 다시 찍어 28장을 채웠다(desktop 픽셀 동일 · mobile 0.004%).

### 19-6. 보고 사항

1. **채용 공고는 3건이다** — 지시서의 「1건」은 2026-06-18 에 2건이 더 등록된 사실이 반영되지 않은 값이다.
   3건 모두 상시채용(`deadline = null`)이라 이번 마감 필터의 영향을 받지 않는다.
2. **채용 공고 관리는 이미 있던 기능이다.** 이번에 새로 넣은 것은 「마감일 경과 자동 제외」와
   관리자 화면의 마감 상태 표기다. 계약 문구 「등록·수정·마감」 중 앞의 둘은 6월에 이미 이행돼 있었다.
3. **게시판 카테고리는 이름·설명까지다**(결정 B). 추가·삭제는 스키마 제약상 불가하고,
   순서·표시숨김은 「메뉴 구성」과 두 출처가 되므로 넣지 않았다. **필요하면 말해 달라.**
4. **낙관적 잠금(E-8)은 채용 공고·게시판 글에 없다.** 기존 코드를 그대로 두었다.
   단지·사이트 설정에만 걸려 있다. 범위 밖이라 판단했으나 확장은 어렵지 않다.
5. **검증 스크립트가 프로덕션 DB(같은 Supabase 프로젝트)에 썼다.** 공고 2건 생성·삭제,
   공지 고정 1건 왕복, 게시판 오버레이 1건 왕복, 단지 1건 수정·복원.
   **전부 원복했고 원복을 DB·SSR 양쪽으로 대조했다**(§19-4 잔존물).
6. **`site_settings` 에 `boardCategories` 행이 생겼다**(값 `{}`). 시드 딥 비교가 이 키를
   「원본에 없는 키」로 잡길래 오버레이 전용 키 예외를 넣었다 — 관리자가 게시판 이름을 바꾼 뒤에도
   시드 검증이 계속 통과해야 하기 때문이다.

---

## 20. DAY 10 실행 결과 (2026-08-26) — 배포 직전 정지

> 지시 10-1 ~ 10-4. **`vercel --prod` 는 실행하지 않았다**(§11-1 · 지시서 「최종 보고 후 정지」).

### 20-1. 회귀 전수 검증

| # | 항목 | 결과 |
|---|------|------|
| 1 | SSR 가시 텍스트 전문 diff | **20경로 전부 0줄** (§19-3) |
| 2 | 회귀 스크린샷 28장 | **크기 불일치 0** · 픽셀 동일 14장 · 나머지 지연 로딩 아티팩트 (§19-5) |
| 3 | 빌드 라우트 표 | **56 → 58.** 추가는 `/admin/content/boards`·`/admin/content/revisions` **2개뿐**. 기존 라우트 `○`/`ƒ` 판정 변화 **0**. `/sitemap.xml` **`○ Static` 유지** |
| 4 | 어댑터 3모드 | `db` · `file`(킬스위치) · `broken`(폴백 강제) — **전부 파일 원본과 불일치 0건** |
| 5 | 킬스위치 전체 렌더 | `CONTENT_SOURCE=file` 서버(:3211) ↔ DB 모드 서버(:3210), **20경로 diff 0줄** |
| 6 | 시드 딥 비교 | complexes 172행 · site_settings 19키 **코드포인트 단위 전부 일치** |
| 7 | slug 일치 | **172/172** |
| 8 | sitemap | **191 URL** · `cases/` 항목 **153** (활성 단지 = DB 153, 과거 19 누출 0) |
| 9 | `tsc --noEmit` | EXIT **0** |
| 10 | `eslint`(신규·변경 전량) | 신규 error·warning **0건**. `app/admin/posts/actions.ts:12 resolveMime` 미사용 경고 1건은 **HEAD 에도 있던 기존 경고**(stash 후 재현 확인) |
| 11 | `npm run build` | EXIT **0** |

**E-1 ~ E-14 최종 상태 표는 `docs/DELIVERY.md` §4 에 정리했다.**
요약: 해소 11 · 부분 해소 1(E-8, 채용·게시판 미적용) · 관리됨 2(E-7 의도된 불일치 · E-11 자산 용량).
**배포 후 확인이 남은 항목 1건** — Vercel 에서의 태그 무효화 → 정적 사이트맵 전파(§11-5 · `docs/DEPLOY.md` §3-3).

### 20-2. 인수 문서 3종

| 문서 | 내용 |
|------|------|
| `docs/DEPLOY.md` | 배포 전 확인(환경변수 6종 · Vercel 환경변수 · 마이그레이션 10종 존재 확인 명령 · 빌드) → 실행 명령 → **배포 직후 체크리스트 8항목**(메인 카운터 200+ / 히어로 8슬라이드 / `/cases` 153+19 / 관리자 로그인 / 편집→반영 실측 / sitemap 191 / **updateTag 즉시 반영** / 채용 3건) → **롤백 2단계**(Vercel Instant Rollback · `CONTENT_SOURCE=file` 킬스위치) |
| `docs/ADMIN_MANUAL.md` | 비개발자용. 10개 항목(로그인 / 단지 / 회사 정보·수치 / 이미지·히어로 / 섹션 / 메뉴·페이지 / 채용 / 게시판·공지 고정 / 복원 / FAQ) + 항목마다 주의사항. 「운영 단지 표기값은 자동으로 안 바뀝니다」 등 함정을 명시 |
| `docs/DELIVERY.md` | 계약 4개 ITEM 이행 내역 표 · **별도 안건 8건** · 검증 근거 · E-1~E-14 최종 상태 · 인수 조건 재기재 |

### 20-3. 별도 안건 (계약 범위 밖 — 결정 대기)

`docs/DELIVERY.md` §2 와 같은 내용이다. 이 문서에는 목록만 남긴다.

1. **Pretendard 미적용** — 라이브는 처음부터 받은 적이 없다. 적용하면 전 페이지 렌더가 변하고 베이스라인 28장 재수립이 필요하다(§15-0).
2. **`/images/hero` 미사용 자산 약 37MB** — 배포에는 이미 제외돼 있다(`.vercelignore`). 저장소에만 남아 있다.
3. **미참조 Storage 객체** — 이미지 교체로 버려진 객체. 되돌리기 수단이라 의도적으로 남긴다.
4. **`businessGallery` 공유 구조** — 사업영역 5종이 갤러리 사진 묶음을 공유한다.
5. **비공개는 연쇄되지 않는다** — 사용자 결정(2026-08-25)으로 **현 동작 유지 확정**.
   계약 문구는 「페이지 공개·비공개 전환」이고 개별 토글이 그 이행이다. 연쇄를 넣으면
   `/business` 하나를 잘못 꺼서 하위 5종이 한꺼번에 사라지는 사고 경로가 생긴다.
   **필요 시 레지스트리에 부모-자식 관계 1줄 확장으로 가능.**
...
## 22. `revalidatePath("/sitemap.xml")` 조치 시도 — **효과 없음** (2026-08-26)

> §21-3 에서 드러난 「sitemap 즉시 반영 안 됨」에 대한 조치. 사용자 지시로 실행했다.
> **결론: 고쳐지지 않았다.** 지시대로 더 시도하지 않고 이 상태로 납품한다.

### 22-1. 조치 내용

`app/admin/content/pages/actions.ts` 의 `togglePagePublished` 에 **1줄 추가**.
기존 `updateTag(CONTENT_TAGS.pages)` 는 그대로 두고 병행한다.

```ts
updateTag(CONTENT_TAGS.pages);
revalidatePath("/sitemap.xml");   // ← 추가
```

- 경로 **1개**만 지정했다. `revalidatePath("/", "layout")` 광역 무효화는 쓰지 않았다(E-12).
- **다른 Server Action 에는 넣지 않았다.** sitemap 에 영향을 주는 액션은 이것 하나뿐이다
  (`grep -rn "revalidatePath" app/admin/content/ lib/` 로 확인 — 나머지 4개 액션은 「쓰지 않는다」 주석만 있다).

### 22-2. 재배포

| 항목 | 값 |
|------|-----|
| 배포 ID | `dpl_7tLisAFT1m6rt2AghkMTBov8CbCw` · **READY** · target production |
| 빌드 | EXIT **0** · 라우트 **58** · `/sitemap.xml` **`○ Static · 1h`** 유지 |
| 별칭 | `kbgroup.kr` · `www.kbgroup.kr` · `kbgroup-renewal.vercel.app` 전부 전환 확인 |

### 22-3. 라이브 실측 — 양방향

1회차의 「sitemap 191 복귀 PASS」는 **애초에 190 으로 내려간 적이 없어 무의미했다.**
그래서 검증 스크립트를 고쳐 **두 방향을 각각 실측**하고, 한쪽이 안 되면 다른 쪽도 PASS 가 되지 않게 묶었다.
sitemap XML 안의 `/licenses` 항목 존재 여부도 건수와 별도로 확인한다.

| 항목 | 결과 |
|------|------|
| ⑦-1 비공개 → `/licenses` **404** | **PASS · 1초** |
| ⑦-2 헤더·푸터 메뉴에서 제외 | **PASS** |
| ⑦-3 비공개 → sitemap 191 → 190 | **FAIL — 191 그대로** (90초 폴링) |
| ⑦-3b sitemap 에서 `/licenses` 항목 소멸 | **FAIL — 남아 있음** |
| ⑦-4 공개 원복 → `/licenses` **200** | PASS |
| ⑦-5 공개 원복 → sitemap 190 → 191 | **FAIL** (내려간 적이 없어 판정 불가) |
| ⑦-5b sitemap 에 `/licenses` 항목 복귀 | PASS |

**`revalidatePath("/sitemap.xml")` 는 프로덕션에서 효과가 없었다.**
어느 캐시 층(Next 정적 라우트 캐시 / Vercel CDN)이 stale 을 주는지는 **확인하지 않았다** —
지시가 「더 시도하지 말라」였고, 원인 층을 알아도 허용된 조치가 남아 있지 않다.

**코드는 되돌리지 않았다.** 의도가 옳고 부작용이 없으며, Next/Vercel 쪽 동작이 바뀌면 그때 걸린다.
다만 소스 주석을 「효과 없음」으로 정정했다 — 동작하는 것처럼 읽히면 안 된다.
(주석 정정은 런타임에 영향이 없어 재배포하지 않았다. 배포된 코드와 HEAD 의 차이는 주석뿐이다.)

### 22-4. 남은 영향과 문서화

- 페이지를 비공개로 돌려도 sitemap 에는 **최대 1시간** 그 URL 이 남는다.
- 해당 URL 로 들어오면 **404 는 즉시** 걸린다. 메뉴에서도 즉시 빠진다.
- 실피해는 **검색엔진이 잠시 죽은 링크를 보는 정도**다.
- `docs/DEPLOY.md` §3-3 · `docs/DELIVERY.md` §4 에 알려진 동작으로 명시했다.

### 22-5. 부수 확인 — 관리자 접근 이상은 재현되지 않았다

이번 라이브 검증에서 **④ 관리자 탭 11개** 가 1회 FAIL 했다(`/admin` 이 `/` 로 리다이렉트).
같은 실행의 뒤쪽 관리자 조작(⑦ 페이지 토글)은 정상 동작했으므로 세션이 죽은 것은 아니다.

- `profiles` 확인: `inspect-admin@kbgroup.kr` → `role=admin` · `status=approved` **정상**
- 앱과 동일한 조회 재현(실제 세션 토큰으로 `profiles` SELECT): **정상 반환**
- `/admin` 재접속 **3회 연속 탭 11개** — **재현되지 않는다**

재배포 직후 첫 요청에서만 났고 이후 재현되지 않아 **원인은 단정하지 않는다.**
제품 결함으로 볼 근거는 없으나, 운영 중 같은 증상이 보이면 이 기록을 참고한다.
